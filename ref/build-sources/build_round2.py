...
SRC = pathlib.Path.home() / "workspace/party-line-assets/research-round2.md"
...
<title>The SI Party Line — Round Two: Conversation Layouts</title>
<style>
...
<header><h1>☎ The SI Party Line — round two: conversation layouts</h1>
<p class="sub">{total} verified conversation-window screenshots · The Palace → Comic Chat → Hotline → MajorBBS → ircII → Yahoo Chat → AOL<br>
Genuine message/conversation windows only — the back-and-forth text. Every entry individually verified by viewing on 2026-09-24. URLs hotlinked as found; some CDN links may rot — every card links its source page where one was retained.</p></header>
{''.join(cards)}
{gaps_html}
...
(OUT / "gallery-round2.html").write_text(gallery)

# ---------- catalog.md ----------
cat_md = ["# The SI Party Line — Visual Research Catalog, Round Two: Conversation Layouts\n",
          f"*{total} entries · researched and individually verified 2026-09-24 · images hotlinked in `gallery-round2.html` (same folder) · companion to `catalog.md` (numbering continues from #44)*\n"]
for cat in categories:
    cat_md.append(f"\n## {cat['title']}\n")
    if cat["note"]:
...
if gaps_block:
    ...
    for ln in gaps_block.split("\n"):
        if ln.strip().startswith("- "):
            cat_md.append(ln + "\n")
(OUT / "catalog-round2.md").write_text("".join(cat_md))
